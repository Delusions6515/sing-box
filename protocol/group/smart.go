...
var (
	errSmartNoLeafCandidates                      = E.New("smart group has no leaf candidates")
...
func (s *Smart) Start() error {
	...
	if err := s.rebuildCandidates(""); err != nil && (len(s.providers) == 0 || !errors.Is(err, errSmartNoLeafCandidates)) {
		return err
	}
...
func (s *Smart) rebuildCandidates(updatedProvider string) error {
	...
	if len(candidates) == 0 {
		return errSmartNoLeafCandidates
	}
