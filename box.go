...
func New(options Options) (*Box, error) {
	...
	if options.Smart != nil || common.Any(options.Outbounds, func(outbound option.Outbound) bool {
		return outbound.Type == C.TypeSmart
	}) {
		smartService = smartservice.NewService(ctx, logFactory.NewLogger("smart"), common.PtrValueOrDefault(options.Smart))
...
func (s *Box) preStart() error {
	...
	if s.smartService != nil {
		err = adapter.StartNamed(s.logger, adapter.StartStateStart, []adapter.LifecycleService{s.smartService})
		if err != nil {
			return err
		}
	}
